...
import org.opengsi.weka.ClassifierNode;
...
public class ClassifierTree implements Drawable, Serializable {
    ...
    public List<ClassifierNode> toNodeList() throws Exception {
        ClassifierNode [] theNodeArray = new ClassifierNode[this.numNodes()];
        for(int i = 0;i<theNodeArray.length;i++) {
            theNodeArray[i] = new ClassifierNode();
        }

        this.assignIDs(-1);
        if (this.m_isLeaf) {
            theNodeArray[m_id].setID(this.m_id);
            theNodeArray[m_id].setName(this.m_localModel.dumpLabel(0, m_train));
        } else {
            theNodeArray[m_id].setID(this.m_id);
            theNodeArray[m_id].setName(m_localModel.leftSide(m_train));
        }

        toNodeList(theNodeArray);
        List<ClassifierNode> theNodeList;
        theNodeList = new ArrayList<ClassifierNode>(Arrays.asList(theNodeArray));

        return theNodeList;

    }

    public void toNodeList(ClassifierNode [] theNodeArray) throws Exception {
        for (int i = 0; i < m_sons.length; i++) {
            int theChildID = m_sons[i].m_id;
            String theCriterion = m_localModel.rightSide(i, m_train).trim();
            theNodeArray[m_id].setID(m_id);
            theNodeArray[m_id].setName(m_localModel.leftSide(m_train)); 
            theNodeArray[m_id].setCriterion(m_localModel.getSplitPoint()); 
            theNodeArray[m_id].setIndex(this.m_train.attribute(theNodeArray[m_id].getName()).index());
            if(theCriterion.contains("<=")) {
                theNodeArray[m_id].setLowID(theChildID);             
            } else {
                theNodeArray[m_id].setHighID(theChildID);                             
            }  
            
            if (m_sons[i].m_isLeaf) {
                theNodeArray[m_sons[i].m_id].setID(m_sons[i].m_id);
                theNodeArray[m_sons[i].m_id].setName(m_localModel.dumpLabel(i, m_train));
            } else {
                m_sons[i].toNodeList(theNodeArray);                
            }
            /*
            text.append("N" + m_id
                    + "->"
